...
begin
      edtPeso.MaxLength := 5;
      if  TryStrToFloat(edtPeso.text, valor3) then
     begin
       if Valor3 > 300 then
       begin
         ShowMessage(' Rasputia? ');
         edtPeso.text := '299';
       end;
     end
     else begin
...
begin
    If (edtIdade.text = '') and (edtAltura.text = '') and (edtPeso.text = '') then
      begin
    ...
    if (resultado >=18.6) and (resultado <=24.999)  then
      MessageDlg(' Você tem '+ edtIdade.text + ' anos, seu IMC é '
       + FormatFloat('0.00', resultado) + ' e você está no seu peso ideal! ',
        TMsgDlgType.mtInformation, [mbOK], 0);


    if (resultado >=25) and (resultado <=29.999)  then
      MessageDlg(' Você tem '+ edtIdade.text + ' anos, seu IMC é '
       + FormatFloat('0.00', resultado) + ' e você está Sobrepeso! ',
        TMsgDlgType.mtInformation, [mbOK], 0);


    if (resultado >=30) and (resultado <=34.999)  then
      MessageDlg(' Você tem '+ edtIdade.text + ' anos, seu IMC é '
       + FormatFloat('0.00', resultado) + ' e você está com obesidade grau I! ',
        TMsgDlgType.mtInformation, [mbOK], 0);


    if (resultado >=35) and (resultado <=39.999)  then
      MessageDlg(' Você tem '+ edtIdade.text + ' anos, seu IMC é '
